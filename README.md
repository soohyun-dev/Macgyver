...
<br><br>
## 2022년 5월 30일
- 시나리오1, 체크리스트 기능 구현 완료. (체크리스트 결과에 따라 호출값 변경 완료)
- 신규 유저는 추천페이지 클릭시 바로 체크리스트로 이동, 기존 유저는 캠핑장 정보 뜨게 구현 완료.
- 북마크 색깔 변경 구현
- 시나리오2 구현 (플라스크에서 배포후 GET)
- 시나리오1 (신규 유저), 시나리오2 (기존 유저) 구분해서 fetch 실행 구현
- 캠핑장 기본이미지 추가 (로고)

<br><br>
## 2022년 6월 1일
- 커뮤니티 글 백엔드 서버로부터 GET, 분류 작업
- category 값으로 분류, 1: 리뷰, 2: 팁, 3: 자유, 4: 신고방, 5: 공지
